...
public class IngestEventControllerTests {
  ...
  class EmptyPayloadTests {
    ...
    void shouldReturn400ForEmptyArrayPayload() throws Exception {
      mockMvc.perform(post("/ingest").contentType(MediaType.APPLICATION_JSON)
  ...
  class ValidPayloadTests {
    ...
    private EventRequest createValidEvent() {
      return new EventRequest(EventType.PAGE_VIEW,
          "e9c0494b2b14ca2b48258c05dd6c4c14",
          new EventRequest.ClientInformation(null, null, null, null,
              null),
          Map.of("page", "/home"));
    }
    ...
    void shouldReturn200ForSingleValidEvent() throws Exception {
      List<EventRequest> events = Collections.singletonList(createValidEvent());
      String payload = objectMapper.writeValueAsString(events);

      mockMvc.perform(post("/ingest").contentType(MediaType.APPLICATION_JSON)
          .content(payload)).andExpect(status().isOk())
          .andExpect(content()
              .string("Events ingested successfully: 1"));
    }
    ...
    void shouldReturn200ForMultipleValidEvents() throws Exception {
      List<EventRequest> events = List.of(createValidEvent(),
          new EventRequest(EventType.USER_ACTION, // Changed to
              // USER_ACTION as
              // "CLICK" is
              // not a direct enum
              // value
              "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6",
              new EventRequest.ClientInformation(null,
                  null, null, null, null),
      ...
      mockMvc.perform(post("/ingest").contentType(MediaType.APPLICATION_JSON)
          .content(payload)).andExpect(status().isOk())
          .andExpect(content()
              .string("Events ingested successfully: 2"));
    }
  }
  ...
  class WrongInputPayloadValidation {
    ...
    void shouldReturn400ForEmptyEventTypeInEvent() throws Exception {
      ...
      EventRequest invalidEvent = new EventRequest(null, // EventType cannot be an
          // empty string, setting
          // to null to trigger
          // @NotNull
          "e9c0494b2b14ca2b48258c05dd6c4c14",
          new EventRequest.ClientInformation(null, null, null, null,
              null),
          Map.of("page", "/home"));
      List<EventRequest> events = Collections.singletonList(invalidEvent);
      String payload = objectMapper.writeValueAsString(events);
    ...
    void shouldReturn400ForEmptyUserHashInEvent() throws Exception {
      EventRequest invalidEvent =
          new EventRequest(EventType.PAGE_VIEW, "",
              new EventRequest.ClientInformation(null,
                  null, null, null, null),
              Map.of("page", "/home"));
      List<EventRequest> events = Collections.singletonList(invalidEvent);
    ...
    void shouldReturn400ForInvalidUserHashInEvent() throws Exception {
      EventRequest invalidEvent =
          new EventRequest(EventType.PAGE_VIEW, "invalid-hash-format",
              new EventRequest.ClientInformation(null,
                  null, null, null, null),
              Map.of("page", "/home"));
      List<EventRequest> events = Collections.singletonList(invalidEvent);
  ...
  class PayloadContentSizeValidation {
    ...
    void shouldReturn400ForBatchSizeExceedingLimit() throws Exception {
      List<EventRequest> events = new java.util.ArrayList<>();
  ...
  class PayloadSizeValidationTests {
    ...
    void shouldReturn400ForPayloadSizeExceedingLimit() throws Exception {
      ...
      for (int i = 0; i < targetSize / 10; i++) { // Divide by 10 for key/value
        // pairs
        sb.append("abcdefghij");
      }
      largeData.put("large_field", sb.toString());

      EventRequest largeEvent =
          new EventRequest(EventType.PAGE_VIEW,
              "e9c0494b2b14ca2b48258c05dd6c4c14",
              new EventRequest.ClientInformation(null,
                  null, null, null, null),
              largeData);
      List<EventRequest> events = Collections.singletonList(largeEvent);
      String payload = objectMapper.writeValueAsString(events);

      mockMvc.perform(post("/ingest").contentType(MediaType.APPLICATION_JSON)
          .content(payload)).andExpect(status().isBadRequest())
          .andExpect(content()
              .string("Payload size cannot exceed 50KB"));
    }
  }
}
